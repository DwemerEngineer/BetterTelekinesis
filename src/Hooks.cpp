	...
	struct Hooks
	{
		struct LimitTelekinesisSound1
		{
			static void thunk(uintptr_t a_arg, intptr_t b_arg, uint32_t c_arg, intptr_t d_arg, int e_arg)
			{
				if (Config::TelekinesisMaxObjects > 1 || !Config::TelekinesisGrabObjectSound) {
					...
					if (now - BetterTelekinesisPlugin::_last_tk_sound2 < 100) {
						return;
					}

					BetterTelekinesisPlugin::_last_tk_sound2 = now;
				}
		...
		struct LimitTelekinesisSound2
		{
			static void thunk(uintptr_t a_arg, intptr_t b_arg, uint32_t c_arg, intptr_t d_arg, int e_arg)
			{
				if (Config::TelekinesisMaxObjects > 1 || !Config::TelekinesisLaunchObjectSound) {
					...
					auto now = GetTickCount();
					if (now - BetterTelekinesisPlugin::_last_tk_sound < 200) {
						return;
					}
					BetterTelekinesisPlugin::_last_tk_sound = now;
				}
				func(a_arg, b_arg, c_arg, d_arg, e_arg);
			}
			static inline REL::Relocation<decltype(thunk)> func;
			...
			static void thunk(uintptr_t a_arg, RE::TelekinesisEffect** a_effect)
			{
				if (Config::DontLaunchIfRunningOutOfMagicka || Config::LaunchIsHotkeyInstead || Config::ThrowActorDamage > 0.0f) {
					...
					if (a_effect != nullptr) {
						...
						if (ef != nullptr && BetterTelekinesisPlugin::IsOurSpell(ef->GetBaseObject()) == BetterTelekinesisPlugin::OurSpellTypes::SwordBarrage) {
							func(a_arg, a_effect);
							return;
						}
					}

					if (BetterTelekinesisPlugin::dropTimer.has_value()) {
						if (GetTickCount() - BetterTelekinesisPlugin::dropTimer.value() < 200) {
						...
						BetterTelekinesisPlugin::dropTimer.reset();
					}
				...
				if (Config::DontLaunchIfRunningOutOfMagicka || Config::LaunchIsHotkeyInstead || Config::ThrowActorDamage > 0.0f) {
					if (BetterTelekinesisPlugin::dropTimer.has_value()) {
						if (GetTickCount() - BetterTelekinesisPlugin::dropTimer.value() < 200) {
							...
							BetterTelekinesisPlugin::OnLaunchActor(actor);
		...
		struct ApplyOverwriteTargetPick
		{
			static void thunk(RE::CrosshairPickData* a_arg, RE::bhkWorld* hkWorld, RE::NiPoint3* sourcePoint, RE::NiPoint3* sourceRotation)
			{
				...
				RE::RefHandle chosenTelekinesis = origTelekinesis;
				{
					...
					if (BetterTelekinesisPlugin::ShouldUpdateTelekinesis(GetTickCount())) {
						BetterTelekinesisPlugin::telekinesisPicked.clear();
						BetterTelekinesisPlugin::grabactorPicked.clear();
						uintptr_t bgt = 0;

						if (Config::DebugLogMode) {
							if (BetterTelekinesisPlugin::profileTimer != nullptr) {
								bgt = BetterTelekinesisPlugin::profileTimer->elapsed<>();
							}
						}

						BetterTelekinesisPlugin::OverwriteTelekinesisTargetPick();

						if (Config::DebugLogMode) {
							if (BetterTelekinesisPlugin::profileTimer != nullptr) {
								BetterTelekinesisPlugin::totalTelekTime += BetterTelekinesisPlugin::profileTimer->elapsed<>() - bgt;

								if (BetterTelekinesisPlugin::timesTelekTime++ % 10 == 1) {
									float telekDistance;
									if (!REL::Module::IsVR()) {
									...
									logger::debug(fmt::runtime("profiler: {.2f} <- {} ; {f}"), static_cast<double>(BetterTelekinesisPlugin::totalTelekTime) / 100 / static_cast<double>(BetterTelekinesisPlugin::timesTelekTime), BetterTelekinesisPlugin::timesTelekTime, telekDistance);
								}
							}
						}
					}
					...
					case 0:
						if (chosenTelekinesis != 0 && std::ranges::find(BetterTelekinesisPlugin::telekinesisPicked.begin(), BetterTelekinesisPlugin::telekinesisPicked.end(), chosenTelekinesis) == BetterTelekinesisPlugin::telekinesisPicked.end()) {
							chosenTelekinesis = 0;
						}
						if (chosenTelekinesis != 0 && !BetterTelekinesisPlugin::HasAnyNormalTelekInHand()) {
							chosenTelekinesis = 0;
						}
						break;

					case 1:
						if (!BetterTelekinesisPlugin::telekinesisPicked.empty()) {
							chosenTelekinesis = BetterTelekinesisPlugin::telekinesisPicked[0];

							auto objRef = RE::TESObjectREFR::LookupByHandle(chosenTelekinesis).get();
							if (objRef == nullptr || BetterTelekinesisPlugin::IsOurItem(objRef->GetBaseObject()) != BetterTelekinesisPlugin::OurItemTypes::None) {
								chosenTelekinesis = 0;
							}
		...
		struct ApplyOverwriteTargetPickVR
		{
			static void thunk(RE::CrosshairPickData* a_arg, RE::bhkWorld* hkWorld, RE::NiPoint3* sourcePoint, RE::NiPoint3* sourceRotation, int b_arg)
			{
				...
				auto hand = BetterTelekinesisPlugin::CastingLeftHandVR() ? RE::VR_DEVICE::kLeftController : RE::VR_DEVICE::kRightController;
				...
				RE::RefHandle chosenTelekinesis = origTelekinesis;
				{
					...
					if (BetterTelekinesisPlugin::ShouldUpdateTelekinesis(GetTickCount())) {
						...
						if (Config::DebugLogMode) {
								...
								if (BetterTelekinesisPlugin::timesTelekTime++ % 10 == 1) {
									logger::debug(fmt::runtime("profiler: {.2f} <- {} ; {f}"), static_cast<double>(BetterTelekinesisPlugin::totalTelekTime) / 100 / static_cast<double>(BetterTelekinesisPlugin::timesTelekTime), BetterTelekinesisPlugin::timesTelekTime, RE::PlayerCharacter::GetSingleton()->GetPlayerRuntimeData().telekinesisDistance);
								}
							}
						}
					}
			...
			static void thunk(RE::TelekinesisEffect* effect)
			{
				...
				unsigned int actorHandle = 0;
				{
					...
					if (!BetterTelekinesisPlugin::telekinesisPicked.empty()) {
						...
						if (!hasBad) {
							for (auto x : BetterTelekinesisPlugin::telekinesisPicked) {
					...
					if (!BetterTelekinesisPlugin::grabactorPicked.empty()) {
						actorHandle = *BetterTelekinesisPlugin::grabactorPicked.begin();
					}
				}
				...
				if (handleId != 0) {
					{
						...
						if (objRefHold == nullptr || !BetterTelekinesisPlugin::CanPickTelekinesisTarget(objRefHold, effects)) {
							handleId = 0;
						}
					}
				}
				...
				if (!failBecauseMax && BetterTelekinesisPlugin::castingNormal && Config::TelekinesisDisarmsEnemies) {
					auto plr = RE::PlayerCharacter::GetSingleton();
					if (Config::TelekinesisDisarmPerk.empty() || plr->HasPerk(BetterTelekinesisPlugin::disarmPerk)) {
						if (actorHandle != 0) {
							{
								...
								if (objRef != nullptr) {
									...
									if (ac != nullptr) {
										BetterTelekinesisPlugin::DisarmActor(ac);
									}
								}
							}
						}
				...
				if (handleId == 0) {
					BetterTelekinesisPlugin::OnFailPickTelekinesisTarget(efs, failBecauseMax);
				}
		...
		struct PlayerUpdateClear
		{
			static void thunk()
			{
				BetterTelekinesisPlugin::ClearGrabIndex(true);
			}
			static inline REL::Relocation<decltype(thunk)> func;
		};
		...
		struct PlayerRevertClear
		{
			static void thunk()
			{
				BetterTelekinesisPlugin::ClearGrabIndex(false);
			}
			static inline REL::Relocation<decltype(thunk)> func;
		};
		...
		struct ActivateHandlerClear
		{
			static void thunk(RE::PlayerCharacter* plr)
			{
				...
				if (BetterTelekinesisPlugin::currentGrabindex != 0 || grabType != RE::PlayerCharacter::GrabbingType::kNone) {
					BetterTelekinesisPlugin::ClearGrabIndex(false);
				}
			}
			static inline REL::Relocation<decltype(thunk)> func;
		...
		struct SeperateTelekinesis
		{
			static void thunk(int a_arg, RE::NiPoint3& a_origin, RE::NiPoint3& a_direction, bool a_includeCameraOffset)
			{
				...
				auto pt = BetterTelekinesisPlugin::currentGrabindex;
				if (pt == 0) {
					return;
				}
...

				{
					...
					if (BetterTelekinesisPlugin::savedGrabindex.contains(pt)) {
						auto& g = BetterTelekinesisPlugin::savedGrabindex.at(pt);
		...
		struct MainUpdate_Nullsub
		{
			static void thunk()
			{
				BetterTelekinesisPlugin::Update();
				func();
			}
			static inline REL::Relocation<decltype(thunk)> func;
		};
		...
		static void Install()
		{
			...
			if (Config::GrabActorNodeNearest || (!Config::GrabActorNodePriority.empty() && Config::GrabActorNodePriority != "NPC Spine2 [Spn2]")) {
				...
				if (!spl.empty()) {
					BetterTelekinesisPlugin::grabActorNodes = spl;
				}
				...
				if (!spl.empty()) {
					BetterTelekinesisPlugin::excludeActorNodes = std::unordered_set<std::string, Util::case_insensitive_unordered_set::hash>();
					for (auto& x : spl) {
						BetterTelekinesisPlugin::excludeActorNodes.insert(x);
					}
				}
